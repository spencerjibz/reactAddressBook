...
class Storage {
	constructor() {
		...
		this.JsonStruct = function (
			...
			this.members =
				!address && members ? new Set(...members) : undefined
		}
	}
	// clear()
	...
	AddOrg(CompanyName, Email, phoneNumber, website, city, members) {
		...
		if (dups && dups.length > 0) {
			CompanyName =
				CompanyName +
				`-${Math.round(dups.length + Math.random(0, 1) * 100)}`
		}
				...
				Orgs.forEach((e) => {
					let obj = [...this.OrgList].filter((v) => {
						return (
							v &&
							v !== null &&
							JSON.parse(v).fullname.trim() === e.trim()
						)
					})[0]
	...
	RemoveCont(fullname) {
		...
		if (obj && this.ContactList.has(obj)) {
			...
			let Obj = [...this.OrgList]
				.filter(
					(v) => v && JSON.parse(v).members.includes(fullname)
				)
				.forEach((e) => {
