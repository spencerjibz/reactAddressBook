...
class Storage {
  ...
  AddOrg(CompanyName, Email, phoneNumber, website, city, members) {
    ...
    if (dups && dups.length > 0) {
      CompanyName =
        CompanyName + `-${Math.round(dups.length + Math.random(0, 1) * 100)}`;
    }

    // create JSON Object
    let obj = new this.JsonStruct(
      CompanyName,
      phoneNumber,
      undefined,
      Email,
  ...
  UpdateOrg(CompanyName, Email, phoneNumber, website, city, members) {
    ...
    let PrevOrg = [...this.OrgList].filter(
      (v) => v && JSON.parse(v).fullname.trim() === CompanyName.trim(),
    )[0];
  ...
  UpdateContact(fullname, phone, city, email, address, Orgs) {
    ...
    let dups = [...this.ContactList].filter(
      (v) => v && JSON.parse(v).fullname.trim() === fullname.trim(),
    )[0];
    // delete previous
    if (dups) {
      ...
      let contact = new this.JsonStruct(
        fullname,
        phone,
        city,
        email,
      ...
      if (Orgs && Orgs.length > 0) {
        Orgs.forEach((e) => {
          let obj = [...this.OrgList].filter((v) => {
            return (
              v && v !== null && JSON.parse(v).fullname.trim() === e.trim()
            );
          })[0];
  ...
  AddContact(fullname, phone, city, email, address, Orgs) {
    // no duplicate Contact named, check if a member with the sample name exists
    let dups = [...this.ContactList].filter(
      (v) => v && JSON.parse(v).fullname === fullname,
    );
    //console.log(dups,fullname)

    if (dups && dups.length > 0) {
      fullname =
        fullname + `-${Math.round(dups.length + Math.random(0, 1) * 100)}`;
    }
    // create JSON Object
    let contact = new this.JsonStruct(
      fullname,
  ...
  RemoveCont(fullname) {
    ...
    let obj = [...this.ContactList].filter(
      (v) => v && JSON.parse(v).fullname.trim() === fullname.trim(),
    )[0];
    ...
    if (obj && this.ContactList.has(obj)) {
      ...
      let Obj = [...this.OrgList]
        .filter((v) => v && JSON.parse(v).members.includes(fullname))
        .forEach((e) => {
  ...
  updateGlobalState() {
    ...
    if (this.ContactList.size > 0 || this.OrgList.size > 0) {
      ...
      localStorage["addressBook"] = JSON.stringify({
        Contacts: [...this.ContactList].filter((v) => v !== null),
        organisations: [...this.OrgList].filter((v) => v !== null),
      });
